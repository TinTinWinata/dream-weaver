...
export default function CrowdfundModalForm({open, setOpen, post} : TCrowdfundModalFormProps) {
  ...
  return (
    <>
      {transfering && <TransferProxy onFinish={onFinish} amount={getValues('amount')} walletPrincipal={'ca3788107ce144cbfa94b68ff0a8b498b5729a9f68d35cc58739c2f32e96eabd'} />}
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 mt-6">
        <Wallet className='text-white'></Wallet>
        <Input
          className='text-white'
          props={{ ...register("amount", { required: 'Amount must be required', min: { value: 0, message: 'Minimal value is 0' } }), type: 'number', min: 0, max: 100, step: 0.0001 }}
